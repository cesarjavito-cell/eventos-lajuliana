...
function formatSpanishWords(amount) {
  ...
  if (n === 115000) return 'Pesos Ciento Quince Mil';
  if (n === 138000) return 'Pesos Ciento Treinta y Ocho Mil';
  if (n === 23000) return 'Pesos Veintitrés Mil';
...
export async function generateContractPdf({ event, contractData = {}, servicesToShow = [], settings = {} }) {
  ...
  if (isEgresados) {
    return generateEgresadosContractPdf({ event, contractData, servicesToShow, settings, logoDataUrl });
  }

  return generateGeneralContractPdf({ event, contractData, servicesToShow, settings, logoDataUrl });
}

/**
 * 📜 CONTRATO GENERAL (Eventos Sociales / Bodas / 15 años)
 */
async function generateGeneralContractPdf({ event, contractData = {}, servicesToShow = [], settings = {}, logoDataUrl }) {
  const doc = new jsPDF();
  ...
  if (logoDataUrl) {
    try {
      doc.addImage(logoDataUrl, 'JPEG', 14, 4, 22, 22);
    } catch (e) {
      console.warn(e);
    }
  }
  ...
  doc.text('QUINTA LA JULIANA', logoDataUrl ? 40 : 14, 14);

  doc.setTextColor(200, 200, 200);
  doc.setFontSize(8.5);
  doc.setFont(undefined, 'normal');
  doc.text('EVENTOS Y HOSPEDAJE  ·  CONTRATO DE LOCACIÓN DE SERVICIOS', logoDataUrl ? 40 : 14, 21);
  ...
  doc.setFontSize(9.5);
  doc.text(p1Lines, 14, y, { maxWidth: 182, align: 'justify' });
  y += p1Lines.length * 5 + 6;
  ...
  const clauses = [
    ...
    `14. Jurisdicción: Tribunales ordinarios de la Ciudad de Posadas, Misiones.-`,
  ];
  ...
  clauses.forEach((cl) => {
    ...
    if (y + lines.length * 4.5 > 240) {
      doc.addPage();
      y = 20;
    }
    doc.text(lines, 14, y, { maxWidth: 182, align: 'justify' });
  ...
  doc.setFont(undefined, 'bold');
  doc.text(closingLines, 14, y, { maxWidth: 182, align: 'justify' });
  y += closingLines.length * 5 + 24;
...
/**
 * 🎓 CONTRATO DE EGRESADOS (5 Páginas Oficiales con Logo Top-Left, Justificado y Texto Ajustado a 9.5pt)
 */
async function generateEgresadosContractPdf({ event, contractData = {}, servicesToShow = [], settings = {}, logoDataUrl }) {
  const doc = new jsPDF();
  ...
  if (logoDataUrl) {
    try {
      doc.addImage(logoDataUrl, 'JPEG', 14, 12, 28, 22);
    } catch (e) {
      console.warn(e);
    }
  }
  ...
  doc.setFont(undefined, 'normal');
  doc.setTextColor(...ink);
  doc.text(`Posadas, Misiones, ${contractDateStr}`, 196, 20, { align: 'right' });

  // Main Title Center
  doc.setFontSize(14);
  doc.setFont(undefined, 'bold');
  doc.setTextColor(15, 23, 42);
  doc.text('CONTRATO', 115, 28, { align: 'center' });
  ...
  doc.setFontSize(9.5);
  doc.setFont(undefined, 'normal');
  doc.text(headerLines, 14, y, { maxWidth: 182, align: 'justify' });
  ...
  doc.setFont(undefined, 'normal');
  doc.text(declALines, 14, y, { maxWidth: 182, align: 'justify' });
  y += declALines.length * 5 + 4;
  ...
  doc.text(declBLines, 14, y, { maxWidth: 182, align: 'justify' });
  y += declBLines.length * 5 + 4;
  ...
  subItems.forEach((si) => {
    const lines = doc.splitTextToSize(si, 178);
    doc.text(lines, 18, y, { maxWidth: 178, align: 'justify' });
    y += lines.length * 5 + 3;
  });
  ...
  doc.text(cl1Lines, 14, y, { maxWidth: 182, align: 'justify' });
  y += cl1Lines.length * 5 + 5;
  ...
  doc.text(cl2Lines, 14, y, { maxWidth: 182, align: 'justify' });
...
Independientemente de la entrega o no de anticipo “EL LOCADOR” deberá entregar al “LOCATARIO” el recibo o comprobante que ampare el pago de los servicios contratados, en la que hará constar detalladamente el nombre y el precio de cada uno de los servicios proporcionados, esto con la finalidad de que el consumidor pueda verificar en detalle.`;
  ...
  doc.text(cl3Lines, 14, y, { maxWidth: 182, align: 'justify' });
  y += cl3Lines.length * 5 + 6;
...
a) El “LOCATARIO” y “El LOCADOR” están de acuerdo en que un personal desempeñe la función de venta y contralor de tarjetas a efectos de que sólo ingresen al lugar personas que cuenten con las mismas.`;
  ...
  doc.text(cl4HeadLines, 14, y, { maxWidth: 182, align: 'justify' });
  y += cl4HeadLines.length * 5 + 4;

  doc.setFont(undefined, 'bold');
  doc.text('❖ Menores de 5 cinco años no pagan.', 18, y); y += 5;
  doc.text('❖ Menores de 6 a 12 años abonan la mitad del valor de una tarjeta.-', 18, y); y += 5;
  doc.text('❖ Discapacitados no pagan (acreditados con CUD).', 18, y); y += 7;

  doc.setFont(undefined, 'bold');
  doc.setTextColor(...violetDark);
  doc.text(`PRECIO DE TARJETA CENA POR PERSONA: ${formatCurrency(cardValue)} (${cardValueWords})`, 14, y); y += 6;
  doc.text(`PRECIO DE TARJETA DESPUES DE LA CENA: ${formatCurrency(afterCardValue)} (${afterCardValueWords})`, 14, y); y += 5.5;
  doc.setFont(undefined, 'normal');
  doc.setTextColor(...ink);
  const subCuarta = 'No se permitirá el ingreso de menores de 18 años, salvo caso que estén acompañados por uno/s de sus tutores.';
  const subCuartaLines = doc.splitTextToSize(subCuarta, 182);
  doc.text(subCuartaLines, 14, y, { maxWidth: 182, align: 'justify' });
  y += subCuartaLines.length * 5 + 4;

  doc.setFont(undefined, 'bold');
  doc.text(`CIERRE DE VENTA DE TARJETAS: ${salesCloseDateStr}.-`, 14, y); y += 8;
  ...
  doc.setFont(undefined, 'normal');
  doc.text(cl5Lines, 14, y, { maxWidth: 182, align: 'justify' });
  y += cl5Lines.length * 5 + 5;
  ...
  doc.text(cl6Lines, 14, y, { maxWidth: 182, align: 'justify' });
  ...
  const clauses3 = [
    ...
    `• La reserva quedará efectiva una vez que el cliente abone el costo de seña de ${advanceCardsCount} (${advanceWord}) tarjetas del evento que se realizará el día ${eventDateStr}, en caso contrario queda a criterio de EL LOCADOR, el uso de dicha fecha para otro cliente si así lo requiera.-`,
  ...
  doc.setFont(undefined, 'bold');
  doc.text(closingEgresadosLines, 14, y, { maxWidth: 182, align: 'justify' });
  ...
  doc.setFont(undefined, 'normal');
  doc.setTextColor(...ink);
  doc.text('El servicio con el cual EL LOCADOR se compromete a cumplir es el siguiente:', 14, y); y += 7;

  doc.text('• Copa de bienvenida / tragos con o sin alcohol.', 18, y); y += 6;

  doc.setFont(undefined, 'bold');
  doc.text('• CENA:', 18, y); y += 6;

  doc.setFont(undefined, 'normal');
  const menuLines = doc.splitTextToSize(cateringMenuText, 170);
  doc.text(menuLines, 22, y, { maxWidth: 170, align: 'justify' });
  y += menuLines.length * 5 + 5;

  const anexoItems = [
    { title: '• POSTRE: ', text: 'bombón helado o mesa de dulces (tarta de frutas, mini choco torta, brownies, trufas, cheesecake).' },
    { title: '• TORTA: ', text: 'Simbólica para decoración y fotos + torta real.' },
    { title: '• BEBIDAS: ', text: 'Barra de tragos (Daiquiris Frutales con o sin alcohol), agua mineral, gaseosas, agua saborizada. Bebidas con alcohol autorizadas por la Municipalidad para mayores de 18 años.-' },
    { title: '• FINAL DE LA FIESTA: ', text: 'pernil con panes y salsas o Pizzas.-' },
    { title: '• DJ - SONIDO - Animador E Iluminación: ', text: 'para la entrada y presentación de los egresados (el volumen del sonido será acorde a los niveles regidos por la municipalidad).-' },
    { title: '• MOZOS: ', text: 'Servicio de mozos y bartender acorde a la cantidad de comensales, los cuales cuentan con respectivos carnets sanitarios, manipulación de alimentos y bebidas.' },
    { title: '• VAJILLA Y MANTELERÍA: ', text: '(Manteles blancos – vajilla platos blancos – cuchillos tenedores - copas - hieleras - champagneras - platitos y cucharitas de postre).' },
    { title: '• MESA Y SILLAS: ', text: 'vestidas.-' },
    { title: '• DECORACION: ', text: 'La decoración (elección de 3 colores) total del salón cuenta con ambientación mobiliario telas luces, livings alfombra, sector para sesión de fotos. Mesa de egresados. No incluye souvenirs ni centro de mesas.' },
  ];

  anexoItems.forEach((item) => {
    const fullStr = `${item.title}${item.text}`;
    const lines = doc.splitTextToSize(fullStr, 178);
    doc.text(lines, 18, y, { maxWidth: 178, align: 'justify' });
    y += lines.length * 5 + 4;
  });
  ...
  const page5Bullets = [
    '• FOTOGRAFÍA: Servicio de fotografía profesional durante el evento, siendo este entregado en forma digital 1 (una) semana después del evento.',
    '• Personal de Recepción ubicado en la entrada del salón para ubicación de mesas.-',
    '• Área Protegida: CELSO SALUD.',
    '• Seguro contra accidentes: LA SEGUNDA.',
    '• Todo esto a cargo de QUINTA LA JULIANA.',
    '• Personal de limpieza, al momento de la finalización del evento.',
  ];

  page5Bullets.forEach((bullet) => {
  ...
  doc.text(afterBoxLines, 18, y + 6, { maxWidth: 175, align: 'justify' });
  y += 32;
